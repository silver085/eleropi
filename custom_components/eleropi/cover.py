...
from .eleroapi import EleroClient
...
async def async_setup_entry(
    ...
    eleroApi = EleroClient(username="ha_user@local.dns" , password="ha_user")
    await eleroApi.update()
    blinds_map = await eleroApi.get_blinds()

    _LOGGER.debug(f"Blinds: {blinds_map}")
    # Add all entities to HA
    async_add_entities(EleroBlind(blind) for blind in blinds_map)
...
class EleroBlind(CoverEntity):
    ...
    supported_features =  SUPPORT_OPEN | SUPPORT_CLOSE | SUPPORT_OPEN_TILT | SUPPORT_CLOSE_TILT | SUPPORT_STOP
    ...
    def __init__(self, blind) -> None:
        ...
        self._attr_name = self._blind['blind_name']
        self._loop = asyncio.get_event_loop()
        self._api = EleroClient(username="ha_user@local.dns" , password="ha_user")


    async def async_update(self):
        _LOGGER.debug(f"Called update for {self._blind['blind_id']}")
        await self._api.update()
        data = await self._api.get_blind(self._blind['blind_id'])
        _LOGGER.debug(f"Received data from hub: {data}")
        self._blind = data
    ...
    async def async_added_to_hass(self) -> None:
        ...
        _LOGGER.debug("Call for async_added_to_hass")

    async def async_will_remove_from_hass(self) -> None:
        """Entity being removed from hass."""
        # The opposite of async_added_to_hass. Remove any registered call backs here.
        #self._blind.remove_callback(self.async_write_ha_state)
        _LOGGER.debug("Call for async_will_remove_from_hass")
    ...
    def current_cover_position(self):
        ...
        if self._blind['blind_state'] == "TOP":
            return 100
        if self._blind['blind_state'] == "BOTTOM": 0
        if self._blind['blind_state'] == "INTERMEDIATE": 50
        else: return 0

    @property
    def is_closed(self) -> bool:
        """Return if the cover is closed, same as position 0."""
        return self._blind['blind_state'] == "BOTTOM"

    @property
    def is_closing(self) -> bool:
        """Return if the cover is closing or not."""
        return self._blind['blind_state'] == "MOVINGDOWN"

    @property
    def is_opening(self) -> bool:
        """Return if the cover is opening or not."""
        return self._blind['blind_state'] == "MOVINGUP"
    ...
    async def async_open_cover(self, **kwargs: Any) -> None:
       ...
       # await self._blind.set_position(100)
        _LOGGER.debug(f"Called open cover")
        res = await self._api.send_command(self._blind['blind_id'], "open")
        _LOGGER.debug(f"Got {res}")

    async def async_close_cover(self, **kwargs: Any) -> None:
        """Close the cover."""
        #await self._blind.set_position(0)
        _LOGGER.debug(f"Called close cover")
        res = await self._api.send_command(self._blind['blind_id'], "close")
        _LOGGER.debug(f"Got {res}")

    async def async_set_cover_position(self, **kwargs: Any) -> None:
        """Close the cover."""
        #await self._blind.set_position(kwargs[ATTR_POSITION])
        _LOGGER.debug(f"Called set  cover pos {kwargs[ATTR_POSITION]}")
        pos = kwargs[ATTR_POSITION]
        if pos <= 50:
            res = await self._api.send_command(self._blind['blind_id'], "intermediate")
        else:
            res = await self._api.send_command(self._blind['blind_id'], "close")
        _LOGGER.debug(f"Got {res}")
    ...
    async def async_stop_cover(self, **kwargs):
        """Stop the cover."""
        _LOGGER.debug(f"Called stop cover")
        res = await self._api.send_command(self._blind['blind_id'], "stop")
        _LOGGER.debug(f"Got {res}")


    async def async_open_cover_tilt(self, **kwargs):
        """Open the cover tilt."""
        _LOGGER.debug(f"Called open cover tilt")
        res = await self._api.send_command(self._blind['blind_id'], "tilt")
        _LOGGER.debug(f"Got {res}")


    async def async_close_cover_tilt(self, **kwargs):
        """Open the cover tilt."""
        _LOGGER.debug(f"Called close cover tilt")
        res = await self._api.send_command(self._blind['blind_id'], "stop")
        _LOGGER.debug(f"Got {res}")
